...
<div class="relative overflow-x-auto mt-5">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-6 py-3">ID Perusahaan</th>
        ...
        <tbody>
            ...
            <tr
                ...
                <td class="px-6 py-4">{{ $farmasi->nama_perusahaan }}</td>
                <td class="px-6 py-4">{{ $farmasi->alamat_perusahaan }}</td>
                <td class="px-6 py-4">
                        ...
                        <div
                            ...
                            <form action="{{ route('farmasi.destroy', $farmasi->id) }}" method="POST">
                                ...
                                <button type="button" data-modal-target="delete-modal" data-modal-toggle="delete-modal"
                                    data-id="{{ $farmasi->id }}" class="delete-button">
    ...
    @if ($farmasis->hasPages())
    <div class="pagination-container mt-4">
        <ul class="pagination flex justify-center items-center space-x-2">
            {{-- Previous Page Link --}}
            @if ($farmasis->onFirstPage())
            <li class="disabled px-3 py-1">Sebelum</li>
            @else
            <li>
                <a href="{{ $farmasis->previousPageUrl() }}" class="px-3 py-1">Sebelum</a>
            </li>
            @endif

            {{-- Pagination Links --}}
            @foreach ($farmasis->getUrlRange(1, $farmasis->lastPage()) as $page => $url)
            <li>
                <a href="{{ $url }}"
                    class="{{ $page == $farmasis->currentPage() ? 'bg-blue-500 text-white px-3 py-1' : 'bg-white text-blue-500 px-3 py-1' }}">
                    {{ $page }}
                </a>
            </li>
            @endforeach
            ...
            {{-- Next Page Link --}}
            @if ($farmasis->hasMorePages())
            <li>
                <a href="{{ $farmasis->nextPageUrl() }}" class="px-3 py-1">Sesudah</a>
            </li>
            @else
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('confirm-delete').addEventListener('click', () => {
            if (deleteId) {
                ...
                form.method = 'POST';
                form.action = `/dashboard/admin/farmasi/${deleteId}`;
                form.innerHTML = `
                    @csrf
                    @method('DELETE')
                `;
